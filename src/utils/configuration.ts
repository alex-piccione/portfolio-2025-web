...
export class ConfigurationProvider {
  ...
  private static async load(retryCount: number = 0): Promise<Configuration> {
    ...
    try {
      const path =
        import.meta.env.VITE_CONFIGURATION_FILE || DEFAULT_CONFIGURATION_FILE
      if (!path)
        throw new Error("CONFIGURATION_FILE environment variable is missed")
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `HTTP error! Status: ${response.status}, StatusText: ${response.statusText}, Body: ${errorText.substring(0, 200)}...`,
        )
      }
      ...
      console.error(
        `Failed to load configuration (retry: ${retryCount}). ${error}`,
      )
      if (response) {
        const htmlText = await response.text()
        console.error(
          `HTTP Status: ${response.status}, Status Text: ${response.statusText}, Body: ${htmlText.substring(0, 200)}...`,
        )
      }
      ...
      if (retryCount >= MAX_RETRY)
        throw new Error(
          `Failed to load configuration after ${MAX_RETRY} retries. LAst error: ${error}`,
        )
